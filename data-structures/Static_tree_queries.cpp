...
struct static_tree_query{
	...
	int N;
	LCA lca;
	vector<array<node, LOG + 1>> up_path;
	vector<INP> A;
	
	static_tree_query(const vector<vector<int>> &adj,const vector<INP> &A_){
		N = adj.size();
		lca = LCA(adj);
		
		up_path = vector<array<node, LOG + 1>> (N);
		A = A_;
		
		for (int x : lca.tour_list) {
			up_path[x][0].apply(A[x]);
			...
			for (int level = 1; level <= LOG; level++) {
				...
				if (anc >= 0)
					up_path[x][level].merge(up_path[anc][level - 1]);
			}
		}
	}
